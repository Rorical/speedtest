...
const MAX_RANDOM_FILL_CHUNK = 64 * 1024; // crypto.getRandomValues limit per call
...
const fillRandomBytes = (buffer: Uint8Array) => {
  ...
  for (let offset = 0; offset < buffer.length; offset += MAX_RANDOM_FILL_CHUNK) {
    const chunk = buffer.subarray(offset, Math.min(offset + MAX_RANDOM_FILL_CHUNK, buffer.length));
